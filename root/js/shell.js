...
(function(root, $, _) {
  Josh.Shell = function(config) {
    ...
    var _readline = config.readline || 
                    new Josh.ReadLine({history: _history, console: _console});
    var _active = false;
    ...
    var _cmdHandlers = {
      ...
      _empty: {
        exec: function(cmd, args, callback) {
          callback(self.templates.empty_command({cmd: cmd}));
        },
    ...
    var self = {
      ...
      templates: {
        ...
        help:          _.template('<div>'+
                                  ...
                                  '}); %>'+
                                  '</div>'),
        bad_command:   _.template('<div>'+
                                  '<strong>Unrecognized command:&nbsp;'+
                                  '</strong>'+
                                  '<%=cmd%>'+
                                  '</div>'),
        empty_command: _.template('<div><%=cmd%></div>'),
        input_cmd:     _.template('<div id="<%- id %>">'+
                                  '<span class="prompt">'+
                                  '</span>&nbsp;<span class="input">'+
                                  '<span class="left"/><span class="cursor"/>'+
                                  '<span class="right"/></span>'+
                                  '</div>'),
        input_search:  _.template('<div id="<%- id %>">'+
                                  '(reverse-i-search)`'+
      ...
      render: function() {
        ...
        if(!cursor) {
          $(id(_input_id)+' .input .cursor').html('&nbsp;')
                                            .css('textDecoration','underline');
        } else {
          $(id(_input_id)+' .input .cursor').text(cursor)
                                            .css('textDecoration', 'underline');
        }
        ...
        for(var i = 0; i < possible.length; i++) {
          ...
          if(option.slice(0, partial.length) == partial) {
            ...
            if(!common) {
              ...
              _console.log("find common stem for '"+common+
                           "' and '" + option + "'");
      ...
      return handler.completion(cmd, arg, line, function(match) {
        ...
        if(match.suggestions && match.suggestions.length > 1) {
          return renderOutput(self.templates.suggest({
                suggestions: match.suggestionsi
             }), function() {

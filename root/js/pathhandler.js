...
(function(root, $, _) {
  Josh.PathHandler = function(shell, config) {
    ...
    var _shell = shell;
    _shell.templates.not_found = _.template("<div>"+
                                            "<%=cmd%>: <%=path%>: "+
                                            "No such file or directory"+
                                            "</div>");
    _shell.templates.ls        = _.template("<div>"+
    ...
    function commandAndPathCompletionHandler(cmd, arg, line, callback) {
      _console.log("calling command and path completion handler w/ cmd: '"+
                   cmd+"', arg: '"+arg+"'");
      if(!arg) {
        arg = cmd;
      }
    ...
    function pathCompletionHandler(cmd, arg, line, callback) {
      ...
      _console.log("completing children via parent '"+parent+
                   "'  w/ partial '"+partial+"'");
